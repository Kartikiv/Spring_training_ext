...
import com.student.data.beans.XrefInsert;
...
public class StudentDaoImplementation implements StudentDao {
	...
	@Value("${queryForInsertXrefTable}")
	String queryForInsertXrefTable;
	@Value("${queryforStudentDetailsWithMarksAndDepartment}")
	String queryforStudentDetailsWithMarksAndDepartment;
	@Autowired
...
//	(:firstNm,:middleNm,:lastNm,:moblieNo,:address,:cityNm,:townNm,:districtNm,:villageNm,:birthDate,:orgId);
    ...
    @Transactional
	@Override
	public Boolean insertStudentDetailsWithKeyHolder(List<InsertStudentDetailsWithDepartment> insertStudentDetailsWithDepartmentList) {
		KeyHolder keyHolder=new GeneratedKeyHolder();
		int result=0;
		List<XrefInsert> xRefInsertList=new ArrayList<>();
		for(InsertStudentDetailsWithDepartment insertStudentDetailsWithDepartment:insertStudentDetailsWithDepartmentList) {
			
			 result=result+namedParameterJdbcTemplate.update(queryForInsertStudentDetails, new BeanPropertySqlParameterSource(insertStudentDetailsWithDepartment), keyHolder,
					new String[] {"student_id"});
			if(insertStudentDetailsWithDepartment.getDepatmentIdList()!=null && !insertStudentDetailsWithDepartment.getDepatmentIdList().isEmpty()) {
			for(StudentDepartment depId:insertStudentDetailsWithDepartment.getDepatmentIdList()) {
				XrefInsert xRefInsert=new XrefInsert();
				xRefInsert.setStudentId(keyHolder.getKey().longValue());
				xRefInsert.setDepartmentId(depId.getDepartmentId());
				xRefInsert.setOrgId(insertStudentDetailsWithDepartment.getOrgId());
				xRefInsertList.add(xRefInsert);
				
			}
			}
			namedParameterJdbcTemplate.batchUpdate(queryForInsertXrefTable,  SqlParameterSourceUtils.createBatch(xRefInsertList) );
			 
			
		}
		
		
		return result==insertStudentDetailsWithDepartmentList.size();
	}

	@Override
	public List<FetchStudentDetailsWithMarksAndDepartmentNames> studentDetailsWithMarksAndDepartment() {
		return namedParameterJdbcTemplate.query(queryforStudentDetailsWithMarksAndDepartment, new BeanPropertyRowMapper<FetchStudentDetailsWithMarksAndDepartmentNames>(FetchStudentDetailsWithMarksAndDepartmentNames.class));
